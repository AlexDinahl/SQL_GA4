...
with steps as (
                            select
                                        ...
                                        from
                                          `isg-dwh-bigquery.206405060.ga_sessions_*` as m, unnest (hits) h
                                          --inner join (select concat(fullvisitorid,'.',visitid) as sid from `isg-dwh-bigquery.206405060.ga_sessions_*`, unnest (hits) h
...
with final as (
                  with steps as (
                              select
                                        ...
                                        from
                                          `isg-dwh-bigquery.206405060.ga_sessions_*`, unnest (hits) h
                                        where _table_suffix between '20230807' and '20230807'
...
with checkout as ( 
   ...
   select
                                        ...
                                        from
                                          `isg-dwh-bigquery.206405060.ga_sessions_*`,date_range, unnest (hits) h
