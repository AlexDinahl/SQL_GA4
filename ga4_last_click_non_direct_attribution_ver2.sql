...
WITH conversions AS
  ( 
   ...
   currentSessions 
AS
(SELECT user_pseudo_id, ga_session_id,IF(gclid is not null, "google", source) source,IF(gclid is not null, "cpc", medium) medium, min(session_start_timestamp) session_start_timestamp FROM
    (SELECT  user_pseudo_id,  
      (SELECT value.int_value FROM unnest(event_params) WHERE key = 'ga_session_id') as ga_session_id, 
      first_value(collected_traffic_source.manual_source)  OVER (PARTITION BY user_pseudo_id, (SELECT value.int_value FROM unnest(event_params) WHERE key = 'ga_session_id') ORDER BY event_timestamp) as source, 
